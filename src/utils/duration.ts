...
export class Duration {
  static inMilliseconds(duration: string): number {
    return this.parse(duration) * 1000;
  }

  static inSeconds(duration: string): number {
    return this.parse(duration);
  }

  static inMinutes(duration: string): number {
    return this.parse(duration) / ONE_MINUTE;
  }

  static inHours(duration: string): number {
    return this.parse(duration) / ONE_HOUR;
  }

  static inDays(duration: string): number {
    return this.parse(duration) / ONE_DAY;
  }

  static inWeeks(duration: string): number {
    return this.parse(duration) / (7 * ONE_DAY);
  }

  static inYears(duration: string): number {
    return this.parse(duration) / (365 * ONE_DAY);
  }
